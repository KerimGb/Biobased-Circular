...
if ( ! defined( 'ULTIMATE_VERSION' ) ) {
	define( 'ULTIMATE_VERSION', '3.21.0' );
}
	...
	class Ultimate_VC_Addons {
		...
		public function __construct() {
			...
			$bsf_analytics = BSF_Analytics_Loader::get_instance();
                        ...
                        // Migrate old analytics option if present.
                        $this->uavc_maybe_migrate_analytics_tracking();
                }
		...
		public function uavc_plugin_install() {
			...
			if ( empty( $plugin_slug ) ) {
				wp_send_json_error( [ 'message' => __( 'Plugin slug is missing.', 'ultimate_vc' ) ] );
			}
			...
			if ( function_exists( 'wp_ajax_install_plugin' ) ) {
				...
				wp_send_json_error( [ 'message' => __( 'Plugin installation function not found.', 'ultimate_vc' ) ] );
			}
		}
		...
		public function uavc_theme_install() {
			...
			if ( empty( $theme_slug ) ) {
				wp_send_json_error( [ 'message' => __( 'Theme slug is missing.', 'ultimate_vc' ) ] );
			}
			...
			if ( function_exists( 'wp_ajax_install_theme' ) ) {
				...
				wp_send_json_error( [ 'message' => __( 'Theme installation function not found.', 'ultimate_vc' ) ] );
			}
		}
			...
			if ( class_exists( 'Nps_Survey' ) ) {
				\Nps_Survey::show_nps_notice(
					...
					array(
						...
						'show_on_screens'  => array( 'toplevel_page_about-ultimate', 'ultimate_page_ultimate-dashboard', 'ultimate_page_ultimate-smoothscroll', 'ultimate_page_ultimate-scripts-and-styles', 'ultimate_page_bsf-font-icon-manager', 'ultimate_page_bsf-google-font-manager', 'ultimate_page_bsf-google-maps'  ),
						'message'          => array(
							...
							'feedback_content'      => __( 'Could you please do us a favor and give us a 5-star rating on Envato? It would help others choose Ultimate Addons for WPBakery with confidence. Thank you!', 'ultimate_vc' ),
		...
		public function toggle_updater() {
			...
			if (get_option('uavc_activation_redirect')) {
                ...
                /**
                 * Migrates analytics tracking option from 'bsf_analytics_optin' to 'uavc_analytics_optin'.
                 *
                 * @since 3.20.4
                 * @return void
                 */
                public function uavc_maybe_migrate_analytics_tracking() {
                        $old_tracking = get_option( 'bsf_analytics_optin', false );
                        $new_tracking = get_option( 'uavc_analytics_optin', false );

                        if ( 'yes' === $old_tracking && false === $new_tracking ) {
                                update_option( 'uavc_analytics_optin', 'yes' );
                                $time = get_option( 'bsf_analytics_installed_time' );
                                update_option( 'uavc_analytics_installed_time', $time );
                        }
                }
