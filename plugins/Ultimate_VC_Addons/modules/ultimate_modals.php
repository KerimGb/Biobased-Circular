	...
	class Ultimate_VC_Addons_Modals {
		...
		public function modal_shortcode( $atts, $content = null ) {
			...
			if ( 'button' == $ult_modal_box_settings['modal_on'] ) {
				...
				$html .= '<span tabindex="0" ' . $madal_trg_data_list . ' style="' . esc_attr( $style ) . ' ' . esc_attr( $trigger_text_style ) . '" data-class-id="content-' . esc_attr( $uniq ) . '" class="' . esc_attr( $modal_class ) . ' ult-responsive mycust ult-align-' . esc_attr( $ult_modal_box_settings['modal_on_align'] ) . '" ' . $modal_data_class . '>' . $ult_modal_box_settings['read_text'] . '</span>';
			}
			$html .= '</div>';
