...
state=$.isPlainObject(state)?$ush.clone(state):{};$.extend(state,self.getDataOfChange());_history.pushState(state,'',self.buildHref());$usb.trigger('urlManager.changed',state);_$locationHref=_location.href}});$usb.urlManager=new URLManager}(jQuery);!function($,undefined){const _window=window;if(!_window.$usb){return}
...
$.extend(Notify.prototype,{isReady:function(){return!$ush.isUndefined(this.$container)},add:function(message,type){const self=this;var delayAutoClose=4000,$notification=self.$container.clone().removeClass('hidden');if(!!type&&$usbcore.indexOf(type,_NOTIFY_TYPE_)>-1){$notification.addClass('type_'+type)}
if(type!==_NOTIFY_TYPE_.ERROR){$notification.addClass('auto_close').data('handle',$ush.timeout(()=>{$notification.find('.usb_action_notification_close').trigger('click')},delayAutoClose))}
...
$notification.fadeOut('fast',()=>{$notification.remove()})},closeAll:function(){$('.usb-notification',$usb.$body).fadeOut('fast',function(){$(this).remove()})}});$usb.notify=new Notify('.usb-notification')}(jQuery);!function($,undefined){const _window=window;if(!_window.$usb){return}
...
else if(state.oldParams.action===_siteSettings){$('.usb_action_show_add_elms',self.$header).removeClass('hidden');$('.usb_action_go_to_back',self.$header).addClass('hidden')}}});$usb.panel=new Panel('#usb-panel')}(jQuery);!function($,undefined){const _window=window;const ceil=Math.ceil;const max=Math.max;if(!_window.$usb){return}
...
self.$screen.removeClass('resizable');$usbcore.cache('temp').remove('resizeData')}});$usb.preview=new Preview('#usb-preview')}(jQuery);!function($,undefined){const _window=window;if(!_window.$usb){return}
...
return result+''}
$usb.cssGenerator=CSSGenerator}(jQuery);!function($,undefined){const _window=window;if(!_window.$usb){return}
...
const $node=$('link[id='+$usb.config('typography.fonts_id')+']',$usb.iframe.contentDocument);if($node.length){$node.attr('href',self._getGoogleEndpoint(themeOptions))}else{$('head',$usb.iframe.contentDocument).append('<link id="'+$usb.config('typography.fonts_id')+'" rel="stylesheet" href="'+self._getGoogleEndpoint(themeOptions)+'" media="all">')}},_getGoogleEndpoint:function(themeOptions){const self=this;var usedFonts={},config=$usb.config('typography',{}),googleFonts=config.googleFonts||{};var tags=config.tags||[];for(const i in tags){var tag=tags[i],tagProps=themeOptions[tag];if(!$.isPlainObject(tagProps)){continue}
...
if(self.isReloadElm(parentId)){self.reloadElmInPreview(parentId);$usb.history.commitChange(newElmId,_CHANGED_ACTION_.CREATE)}else if(self.isReloadParentElm(newElmId)){self.reloadElmInPreview(self.getElmParentId(newElmId));$usb.history.commitChange(newElmId,_CHANGED_ACTION_.CREATE)}else{self.addElmToPreview(newElmId,index,parentId)}
$ush.timeout(self.updateElementsList.bind(self),1)},_elmDelete:function(id){const self=this;if(!self.isValidId(id)){return}
if(self.isChildElmContainer(id)&&self.getElmSiblingsId(id).length===1){id=self.getElmParentId(id)}
self.removeElm(id)}});$.extend(prototype,{_dragStartDistance:5,_dragstart:function(e){return $(e.target).closest('.media-frame').length>0},initDragDrop:function(){const self=this;if(_$tmp.isInitDragDrop){return}
...
self.removeDragScrollData();$usb.postMessage('doAction','_scrollDragging')}});$.extend(prototype,{updateElementsList:function(){const self=this;const content=$ush.toString(self.pageData.content);const allShortcodesRegexp=self.getShortcodeRegex('(us|vc)?[\dA-z_]+');const parseShortcode=(content,parentId,depth)=>{depth=$ush.parseInt(depth);if(!content||depth>1000){return}
content.matchAll(allShortcodesRegexp).forEach((matches)=>{const shortcodeId=(matches[4].match(/(\s?usbid="([^\"]+)?")/)||[])[2];const shortcodeContent=$ush.toString(matches[6]);if(!shortcodeId){return}
if(shortcodeContent){parseShortcode(shortcodeContent,shortcodeId,depth++)}
elementsList[shortcodeId]=parentId})}
parseShortcode(content,self.mainContainer);return elementsList},isProcessSave:function(){return _$tmp.isProcessSave},savePageData:function(complete){const self=this;var data={postMeta:{},};if(self.isContentChanged()){data.post_content=self.pageData.content}
...
const elmParentId=self.getElmParentId(id);if(isTempContent&&tempContent){self.saveTempContent();self.pageData.content=''+tempContent}
...
const name=self.getElmName(id);return $usb.config(`elm_titles.${name}`)||name},doesElmExist:function(id){const self=this;if(!self.isValidId(id)||elementsList.length===0){return!1}
return!$ush.isUndefined(elementsList[id])},getElmId:function(node){const self=this;if(!$ush.isNode(node)){return ''}
...
return elementsList[id]||null},getElmNextId:function(id){const self=this;if(!self.isValidId(id)||self.isMainContainer(id)){return null}
const children=self.getElmChildren(self.getElmParentId(id));const index=children.indexOf(id);if(index<0||children.length===index){return null}
return children[++index]||null},getElmPrevId:function(id){const self=this;if(!self.isValidId(id)||self.isMainContainer(id)){return null}
const children=self.getElmChildren(self.getElmParentId(id));const index=children.indexOf(id);if(index<0||index===0){return null}
return children[--index]||null},getElmSiblingsId:function(id){const self=this;if(!self.isValidId(id)||self.isMainContainer(id)){return[]}
return self.getElmChildren(self.getElmParentId(id))},getElmChildren:function(id){const self=this;const result=[];if(!id){return result}
for(const k in elementsList){if(elementsList[k]===id){result.push(k)}}
return result},getElmAllChildren:function(id){const self=this;if(!self.isValidId(id)||!self.isElmContainer(id)){return[]}
...
if(columnsCount<newColumnsCount){for(var i=columnsCount;i<newColumnsCount;i ++){const newColumnId=self.getSpareElmId(columnType);self._addShortcodeToContent(rowId,i,`[${columnType} usbid="${newColumnId}"][/${columnType}]`);elementsList[newColumnId]=rowId}
...
return renderNeeded},getInsertPosition:function(parent,index){const self=this;const isRootElmContainer=self.isElmContainer(parent);var position;index=$ush.parseInt(index);if(self.isMainContainer(parent)||isRootElmContainer){var children=self.getElmChildren(parent);if(index===0||children.length===0){position='prepend'}else if(index>children.length||children.length===1){index=children.length;position='append'}else{parent=children[index-1]||parent;position='after'}}else{position=(index<1?'before':'after')}
...
self.updateElementsList();return elmId},moveElm:function(moveId,newParentId,newIndex){const self=this;const args=arguments;if(self.isMainContainer(moveId)){$usb.log('Error: Cannot move the container',args);return!1}
...
self.updateElementsList();$usb.trigger('builder.contentChange');return!0},removeElm:function(removeId){const self=this;if(!self.isValidId(removeId)){return!1}
...
children.push(removeId);for(const k in children){delete elementsList[children[k]]}
...
pastedContent=$usb.builder.removeHtmlWrap(pastedContent);const isValid=!(!/^\[vc_row([\s\S]*)\/vc_row\]$/gim.test(pastedContent)||pastedContent.indexOf('[vc_column')===-1);$textarea.toggleClass('validate_error',!isValid);if(!isValid){$usb.notify.add($usb.getTextTranslation('invalid_data'),_NOTIFY_TYPE_.ERROR);return}
...
$usb.history.commitChange(elmId,_CHANGED_ACTION_.CREATE);$usb.builder.pageData.content+=(res.data.content||pastedContent.replace(/(grid_layout_data="([^"]+)")/g,'items_layout=""'));$usb.postMessage('insertElm',[$usb.builder.mainContainer,'append',res.data.html,!0]);$usb.trigger('builder.contentChange')},complete:(_,textStatus)=>{const isSuccess=(textStatus==='success');$saveButton.prop('disabled',isSuccess).removeClass('loading').toggleClass('disabled',isSuccess);$textarea.prop('disabled',!1).removeClass('disabled');if(isSuccess){$textarea.val('')}}})}});$.extend(prototype,{_loadDeferredFieldsets:function(name){const self=this;const data={};$usb.$panel.addClass('data_loading');var requestId='loadDeferredFieldsets';if(!$ush.isUndefined(name)){data.name=name;requestId+='.name';$usb.$panel.addClass('show_preloader')}
...
if(!_$tmp.elmsFieldset[name]&&!_$tmp.isFieldsetsLoaded){$usb.panel.setTitle(elmTitle);self.one('fieldsetLoaded',(loadedName)=>{if(name!==loadedName){return}
...
self.$activeElmFieldset=$(_$tmp.elmsFieldset[name]);$usb.$panelBody.prepend(self.$activeElmFieldset);$usb.builder.selectedElmId=id;self.activeElmFieldset=new $usof.Form(self.$activeElmFieldset);$usb.panel.setTitle($usb.builder.getElmTitle(id));self.$activeElmFieldset.addClass('usof-container');self.activeElmFieldset.setValues(values,!0);if($usb.find('preview')){$usb.preview.setFieldResponsiveScreen()}
for(const fieldId in self.activeElmFieldset.fields){self.activeElmFieldset.fields[fieldId].on('change',self._events.changeField.bind(self,id)).on('afterHide',self._events.afterHideField).on('changeDesignField',self._events.changeDesignField.bind(self,id)).on('syncResponsiveState',(_,screenName)=>{if($usb.find('preview')){$usb.preview.fieldSetResponsiveScreen(screenName)}}).on('tinyMCE.Keydown',(_,e)=>$usb._events.keydown(e))}
for(const groupName in(self.activeElmFieldset.groups||{})){self.activeElmFieldset.groups[groupName].on('change',$ush.debounce(self._events.changeField.bind(self,id),1))}
...
if(isUpdated){$usb.history.setLatestShortcodeUpdate({content:oldShortcode,preview:$usb.builder.getElmOuterHtml(_currentData.id)});var commitArgs=[_currentData.id,_CHANGED_ACTION_.UPDATE];commitArgs.push($usb.config('useThrottleForFields',[]).includes(_currentData.usofField.type));if(oldParentShortcode){commitArgs.push({originalId:originalId})}
...
if($usb.config('useLongUpdateForFields',[]).includes(_currentData.usofField.type)){self.__updateOnInstructions_long(_updateOnInstructions.bind(self,_currentData))}else{_updateOnInstructions(_currentData)}};if(_currentData.isChangeDesignOptions){const _value=unescape($ush.toString(_currentData.value));const attachmentId=$ush.parseInt((_value.match(/"background-image":"(\d+)"/)||[])[1]);if(attachmentId&&!$usb.getAttachmentUrl(attachmentId)){($usb.getAttachment(attachmentId)||{fetch:$.noop}).fetch({success:_switchUpdateOnInstructions})}else{_switchUpdateOnInstructions()}}else{_switchUpdateOnInstructions()}}},_afterHideField:function(usofField){if(usofField instanceof $usof.field&&usofField.inited){usofField.setValue(usofField.getDefaultValue(),!1)}}});prototype.$$fieldsets={_switchTabs:function(e){const $target=$(e.currentTarget);const $sections=$target.parents('.usof-tabs:first').find('> .usof-tabs-sections > *');$target.addClass('active').siblings().removeClass('active');$sections.removeAttr('style').eq($target.index()).addClass('active').siblings().removeClass('active')},autoShowingTabs:function(){const self=this;if(!self.activeElmFieldset||!self.activeElmFieldset.isGroupParams){return}
$.each(self.activeElmFieldset.$tabsSections,(index,section)=>{var fields=$('> *',section).toArray(),isHidden=!0;for(const k in fields){var $field=$(fields[k]),isShown=$field.data('isShown');if($ush.isUndefined(isShown)){isShown=($field.css('display')!='none')}
if(isShown){isHidden=!1;break}}
self.activeElmFieldset.$tabsItems.eq(index).toggleClass('hidden',isHidden)})}};$usb.builderPanel=new BuilderPanel}(jQuery);!function($,undefined){const _window=window;if(!_window.$usb){return}
...
level++;elmsId.map((elmId)=>{const attrId=$usb.builder.getElmValue(elmId,'el_id','');const $item=$($usb.buildString(_$tmp.template,{attr_id:(attrId?'#'+attrId:''),elm_icon:$usb.config('elm_icons.'+$usb.builder.getElmName(elmId),'no-icon'),elm_title:$usb.builder.getElmTitle(elmId),elm_type:$usb.builder.getElmType(elmId),usbid:elmId,}));var itemChildren=$usb.builder.getElmChildren(elmId);if(itemChildren.length){getItems(itemChildren,$item,level);$item.addClass('has_children').toggleClass('expand',$(`[data-for="${elmId}"].expand`,self.$body).length>0)}
...
if($usb.urlManager.hasParam('navigator','show')){self.show()}else{self.hide()}}});$usb.navigator=new Navigator('#usb-navigator')}(jQuery);!function($,undefined){const _window=window;if(!_window.$usb){return}
...
const newData=$usb.builder.updateIdsInContent(res.data.content,res.data.html);if(!$usb.builder._addShortcodeToContent(parentId,currentIndex,newData.content)){return!1}
...
$ush.timeout(()=>{$usb.builder.updateElementsList()},0);$usb.trigger('builder.contentChange')}})};if(!self.templateIsLoaded(categoryId)){self.off('templatesLoaded').one('templatesLoaded',(_categoryId)=>{if(categoryId==_categoryId){_getTemplateData()}});if(self.categoryIsLoaded(categoryId)){$usb.log('Error: Failed to load template category:',[categoryId])}
return}
...
const newData=$usb.builder.updateIdsInContent(res.data.content,res.data.html);if(!$usb.builder._addShortcodeToContent(parentId,currentIndex,newData.content)){return!1}
$usb.postMessage('insertElm',[insert.parent,insert.position,newData.html]);if($usb.builder.isRow(newData.firstElmId)){$usb.history.commitChange(newData.firstElmId,_CHANGED_ACTION_.CREATE)}
$ush.timeout(()=>{$usb.builder.updateElementsList()},0);$usb.trigger('builder.contentChange')}})},});$usb.favorites=new Favorites('#usb-favorites')}(jQuery);!function($,undefined){const _window=window;if(!_window.$usb){return}
...
setPageCustomCss(pageCustomCss)}});$usb.page=new Page}(jQuery);!function($,undefined){const _window=window;if(!_window.$usb){return}
_window.$ush=_window.$ush||{};_window.$usof=_window.$usof||{};_window.$usbcore=_window.$usbcore||{};const _HISTORY_TYPE_={REDO:'redo',UNDO:'undo'};var _$changesHistory={redo:[],tasks:[],undo:[]};var _$tmp={isActiveRecoveryTask:!1,_latestShortcodeUpdate:{},};function History(){const self=this;self._events={historyChanged:self._historyChanged.bind(self),redoChange:self._redoChange.bind(self),undoChange:self._undoChange.bind(self),};$(()=>{self.$actionUndo=$('.usb_action_undo',$usb.$panel);self.$actionRedo=$('.usb_action_redo',$usb.$panel);$usb.$panel.on('click','.usb_action_undo',self._events.undoChange).on('click','.usb_action_redo',self._events.redoChange)});$usb.on('hotkeys.ctrl+z',self._events.undoChange).on('hotkeys.ctrl+shift+z',self._events.redoChange);self.on('historyChanged',self._events.historyChanged)}
$.extend(History.prototype,$ush.mixinEvents,{_undoChange:function(){this._createRecoveryTask(_HISTORY_TYPE_.UNDO)},_redoChange:function(){this._createRecoveryTask(_HISTORY_TYPE_.REDO)},_historyChanged:function(){const self=this;[{$btn:self.$actionUndo,disabled:!self.getLengthUndo()},{$btn:self.$actionRedo,disabled:!self.getLengthRedo()}].map((i)=>{i.$btn.toggleClass('recovery_process',!!self.getLengthTasks()).toggleClass('disabled',i.disabled).prop('disabled',i.disabled)})},getLengthUndo:function(){return _$changesHistory.undo.length},getLengthRedo:function(){return _$changesHistory.redo.length},getLengthTasks:function(){return _$changesHistory.tasks.length},getLastHistoryDataByAction:function(action){const self=this;var lastData,undo=_$changesHistory.undo;if(self.getLengthUndo()&&$usbcore.indexOf(action,_CHANGED_ACTION_)>-1){for(var i=self.getLengthUndo()-1;i>=0;i--){if((undo[i]||{}).action===action){lastData=$ush.clone(undo[i]);break}}}
return lastData||{}},setLatestShortcodeUpdate:function(data){_$tmp._latestShortcodeUpdate=$ush.toPlainObject(data)},isActiveRecoveryTask:function(){return!!_$tmp.isActiveRecoveryTask},__saveDataToHistory:$ush.throttle($ush.fn,2000,!0),commitChange:function(id,action,useThrottle,extData){const self=this;if(!action||!$usb.builder.isValidId(id)||self.isActiveRecoveryTask()||$usbcore.indexOf(action,_CHANGED_ACTION_)<0){return}
const saveDataToHistory=function(){var data={action:action,id:id,extData:$.isPlainObject(extData)?extData:{},};if($usbcore.indexOf(action,[_CHANGED_ACTION_.MOVE,_CHANGED_ACTION_.REMOVE])>-1){data.index=$usb.builder.getElmIndex(id);data.parentId=$usb.builder.getElmParentId(id)}
...
const saveDataToHistory=function(){var data={action:_CHANGED_ACTION_.CALLBACK,callback:callback,data:customData};var lastData=self.getLastHistoryDataByAction(_CHANGED_ACTION_.CALLBACK);if(!$.isEmptyObject(lastData)&&$ush.comparePlainObject($usbcore.clearPlainObject(lastData,['callback','timestamp']),$usbcore.clearPlainObject(data,'callback'))){return}
if(self.getLengthUndo()>=$ush.parseInt($usb.config('maxDataHistory',100))){_$changesHistory.undo=_$changesHistory.undo.slice(1)}
_$changesHistory.undo.push($.extend(data,{timestamp:Date.now()}));_$changesHistory.redo=[];self.trigger('historyChanged')};if(!!useThrottle){self.__saveDataToHistory(saveDataToHistory)}else{saveDataToHistory()}},_createRecoveryTask:function(type){const self=this;if(!type||$usbcore.indexOf(type,[_HISTORY_TYPE_.UNDO,_HISTORY_TYPE_.REDO])<0){return}
...
if(!$.isEmptyObject(task)){_$changesHistory.tasks.push($ush.clone(task,{_source:type}));self.trigger('historyChanged');self.__startRecoveryTasks.call(self)}},__startRecoveryTasks:$ush.debounce(function(){const self=this;if(self.isActiveRecoveryTask()){return}
_$tmp.isActiveRecoveryTask=!0;self._recoveryTaskManager()},100),_recoveryTaskManager:function(){const self=this;var lengthTasks=self.getLengthTasks(),task=_$changesHistory.tasks[--lengthTasks];if($.isEmptyObject(task)){_$tmp.isActiveRecoveryTask=!1;self.trigger('historyChanged');return}
...
var _source=task._source;delete task._source;if(_source===_HISTORY_TYPE_.UNDO){_$changesHistory.redo.push(task)}else{_$changesHistory.undo.push(task)}},_applyChangesFromTask:function(task,originalTask){const self=this;if($.isEmptyObject(task)){_$tmp.isActiveRecoveryTask=!1;return}
...
self.trigger('historyChanged');$ush.timeout(self._recoveryTaskManager.bind(self),1)}});$usb.history=new History}(jQuery);!function($,undefined){const _window=window;if(!_window.$usb){return}
...
$usb.notify.add($usb.getTextTranslation('site_settings_updated'),_NOTIFY_TYPE_.SUCCESS);$usb.postMessage('clearCookies');_$tmp.savedLiveOptions=$ush.clone(self.liveOptions)},complete:()=>{_$tmp.isProcessSave=!1;$usb.panel.switchSaveButton(!1)}})}});$usb.siteSettings=new SiteSettings('#usb-site-settings')}(jQuery)