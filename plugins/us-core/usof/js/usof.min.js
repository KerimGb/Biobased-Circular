...
history.pushState($.extend(state,self.getChangedParams()),'',lastUrl=self.toString());$window.trigger('pushstate');return self}})}}(jQuery);const _window=window;const _document=document;_window.$ush=_window.$ush||{};_window.$usof=_window.$usof||{};jQuery.fn.usMod=function(mod,value){if(this.length==0){return this}
...
else{var regexp=new RegExp('(^| )'+mod+'\_[a-zA-Z0-9\_\-]+( |$)');return this.each(function(){if(this.className.match(regexp)){this.className=this.className.replace(regexp,'$1'+mod+'_'+value+'$2')}else{this.className+=' '+mod+'_'+value}}).trigger('usof.'+mod,value)}};!function($,_undefined){"use strict";$usof.ajaxUrl=$('.usof-container').data('ajaxurl')||ajaxurl;if($ush.isUndefined($usof.mixins)){$usof.mixins={}}
...
return iconTag}};if($ush.isUndefined($usof._$$data)){$usof._$$data={}}
...
return $ush.clone(self._$$data[key]||{})}}(jQuery);!function($,_undefined){"use strict";$usof.field=function(row,options){const self=this;self.$document=$(_document);self.$row=$(row);self.$responsive=$('> .usof-form-row-responsive',self.$row);const data=self.$row.data()||{};self.type=self.$row.usMod('type');self.id=data.id;self.uniqid=$ush.uniqid();self.name=data.name;self.inited=!!data.inited;self.relatedOn=data.relatedOn;self.$input=$(`[name="${data.name}"]:not(.js_hidden)`,self.$row);if(self.inited){return}
...
const value=self.getValue();$usof.instance.valuesChanged[self.groupName]=value;self.$container.trigger('change',value)}},getDefaultValue:function(){return $ush.isUndefined(this._std)?'':this._std},getCurrentValue:function(){const self=this;var result=[];for(const i in self.groupParams){result.push(self.groupParams[i].getCurrentValues())}
...
groupParams.fields[k].trigger('change');break}});self._reInitGroupParams();self._reInitValuesChanged()},getValue:function(){const self=this;var result=[];$.each(self.groupParams,(_,groupParams)=>result.push(groupParams.getValues()));if(self.hasStringValue){if(result.length){try{result=$ush.toString(result)}catch(err){console.error(result,err);result=self.getDefaultValue()}}else{result=self.getDefaultValue()}}
...
if(self.isFieldPreview){self.$title.attr('placeholder',title)}else{self.$title.text(title)}}})}(jQuery);!function($,_undefined){"use strict";$usof.Meta=function(container){const self=this;_window.USMMSettings=_window.USMMSettings||{};self.$container=$(container);self.initFields(self.$container);self.fireFieldEvent(self.$container,'beforeShow');self.fireFieldEvent(self.$container,'afterShow');for(const fieldId in self.fields){if(!self.fields.hasOwnProperty(fieldId)){continue}
self.fields[fieldId].on('change',(field,value)=>{for(const savingFieldId in self.fields){_window.USMMSettings[savingFieldId]=self.fields[savingFieldId].getValue()}
$(_document.body).trigger('usof_mm_save');})}
self.$container.data('usof.Meta',self)};$.extend($usof.Meta.prototype,$usof.mixins.Fieldset);$(()=>{$.each($('.usof-container.for_meta'),(_,node)=>new $usof.Meta(node));$(_document.body).off('usof_mm_load').on('usof_mm_load',()=>{$('.us-mm-settings').each((_,node)=>new $usof.Meta(node))})})}(jQuery);!function($,_undefined){"use strict";$usof.Form=function(container){const self=this;self.$container=$(container);self.$title=$('.usof-header-title h2',self.$container);self.$container.addClass('inited');$usof.instance=self;self.initFields(self.$container);self.active=null;self.$sections={};self.$sectionContents={};self.sectionFields={};$.each($('.usof-section',self.$container),(index,section)=>{var $section=$(section),sectionId=$section.data('id');self.$sections[sectionId]=$section;self.$sectionContents[sectionId]=$('.usof-section-content',$section);if($section.hasClass('current')){self.active=sectionId}
self.sectionFields[sectionId]=[];$.each($('.usof-form-row',$section),(index,row)=>{const fieldName=$(row).data('name');if(fieldName){self.sectionFields[sectionId].push(fieldName)}})});self.sectionTitles={};$.each($('.usof-nav-item.level_1',self.$container),(index,item)=>{const $item=$(item);self.sectionTitles[$item.data('id')]=$('.usof-nav-title',$item).html()});self.navItems=$('.usof-nav-item.level_1, .usof-section-header',self.$container);self.sectionHeaders=$('.usof-section-header',self.$container);self.sectionHeaders.each((index,item)=>{const $item=$(item);$item.on('click',()=>self.openSection($item.data('id')))});if(_document.location.hash&&_document.location.hash.indexOf('#!')==-1){self.openSection(_document.location.hash.substring(1))}
if(!$ush.isUndefined(self.$sections[self.active])){self.fireFieldEvent(self.$sections[self.active],'beforeShow');self.fireFieldEvent(self.$sections[self.active],'afterShow')}
self.$saveControl=$('.usof-control.for_save',self.$container);self.$saveBtn=$('.usof-button',self.$saveControl).on('click',self.save.bind(self));self.$saveMessage=$('.usof-control-message',self.$saveControl);self.valuesChanged={};self.saveStateTimer=null;for(const fieldId in self.fields){if(!self.fields.hasOwnProperty(fieldId)){continue}
...
self.$window=$(_window);self.$header=$('.usof-header',self.$container);self.$schemeBtn=$('.for_schemes',self.$container);self.$schemeBtn.on('click',()=>$('.usof-form-row.type_style_scheme').show());self._events={scroll:self.scroll.bind(self),resize:self.resize.bind(self)};self.resize();self.$window.on('resize load',self._events.resize);self.$window.on('scroll',self._events.scroll);self.$window.on('hashchange',()=>self.openSection(_document.location.hash.substring(1)));$(_window).on('keydown',(event)=>{if(event.ctrlKey||event.metaKey){if(String.fromCharCode(event.which).toLowerCase()=='s'){event.preventDefault();$usof.instance.save()}}});self.$container.data('usof.Form',self)};$.extend($usof.Form.prototype,$usof.mixins.Fieldset,{scroll:function(){this.$container.toggleClass('footer_fixed',this.$window.scrollTop()>this.headerAreaSize)},resize:function(){const self=this;if(!self.$header.length){return}
self.headerAreaSize=self.$header.offset().top+self.$header.outerHeight();self.scroll()},openSection:function(sectionId){const self=this;if(sectionId==self.active||$ush.isUndefined(self.$sections[sectionId])){return}
if(!$ush.isUndefined(self.$sections[self.active])){self.hideSection()}
self.showSection(sectionId);self.$schemeBtn=$('.for_schemes',self.$container);if(sectionId=='colors'){self.$schemeBtn.removeClass('hidden')}else{self.$schemeBtn.addClass('hidden')}},showSection:function(sectionId){const self=this;const $curItem=self.navItems.filter(`[data-id="${sectionId}"]`);$curItem.addClass('current');self.fireFieldEvent(self.$sectionContents[sectionId],'beforeShow');self.$sectionContents[sectionId].stop(!0,!1).fadeIn();self.$title.html(self.sectionTitles[sectionId]);self.fireFieldEvent(self.$sectionContents[sectionId],'afterShow');const itemPopup=$('.usof-nav-popup',$curItem);if(itemPopup.length>0){const matches=_document.cookie.match(/(?:^|; )usof_visited_new_sections=([^;]*)/);const cookieValue=matches?decodeURIComponent(matches[1]):'';const visitedNewSections=(cookieValue=='')?[]:cookieValue.split(',');if(visitedNewSections.indexOf(sectionId)==-1){visitedNewSections.push(sectionId);_document.cookie='usof_visited_new_sections='+visitedNewSections.join(',')}
itemPopup.remove()}
self.active=sectionId},hideSection:function(){const self=this;self.navItems.filter(`[data-id="${self.active}"]`).removeClass('current');self.fireFieldEvent(self.$sectionContents[self.active],'beforeHide');self.$sectionContents[self.active].stop(!0,!1).hide();self.$title.html('');self.fireFieldEvent(self.$sectionContents[self.active],'afterHide');self.active=null},save:function(){const self=this;if($.isEmptyObject(self.valuesChanged)){return}
clearTimeout(self.saveStateTimer);self.$saveMessage.html('');self.$saveControl.usMod('status','loading');$.ajax({type:'POST',url:$usof.ajaxUrl,dataType:'json',data:{action:'usof_save',usof_options:JSON.stringify(self.valuesChanged),_wpnonce:$('[name="_wpnonce"]',self.$container).val(),_wp_http_referer:$('[name="_wp_http_referer"]',self.$container).val()},success:function(result){if(result.success){self.valuesChanged={};self.$saveMessage.html(result.data.message);self.$saveControl.usMod('status','success');self.saveStateTimer=setTimeout(()=>{self.$saveMessage.html('');self.$saveControl.usMod('status','clear')},4000)}else{self.$saveMessage.html(result.data.message);self.$saveControl.usMod('status','error');self.saveStateTimer=setTimeout(()=>{self.$saveMessage.html('');self.$saveControl.usMod('status','notsaved')},4000)}}})}});$(()=>new $usof.Form('.usof-container:not(.inited)'))}(jQuery);!function($,_undefined){"use strict";$usof.dragDrop=function(container,itemSelector,checkDraggable){let self=this;self.itemSelector=itemSelector;self.checkDraggable=checkDraggable;self.isPlaceAfter=!1;self._events={dragstart:self._dragstart.bind(self),dragover:self._dragover.bind(self),drop:self._drop.bind(self),dragend:self._dragend.bind(self),};self.$container=$(container);self.init()};$.extend($usof.dragDrop.prototype,$usof.mixins.Events,{init:function(){let self=this;self.$container.addClass('usof-dragdrop').data('usof.dragDrop',self).on('mousedown',self.itemSelector,function(e){if(self.checkDraggable&&!e.target.className.includes('usof_draggable')){return!0}
