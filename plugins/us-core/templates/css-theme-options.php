...
*:not(.w-hwrapper) > .w-btn-wrapper:not([class*="default_align_none"]):not(.align_none) {
	display: block;
	margin-inline-end: 0;
	}
}
...
*:not(.w-hwrapper) > .w-btn-wrapper:not([class*="laptops_align_none"]):not(.align_none) {
	display: block;
	margin-inline-end: 0;
	}
...
*:not(.w-hwrapper) > .w-btn-wrapper:not([class*="tablets_align_none"]):not(.align_none) {
	display: block;
	margin-inline-end: 0;
	}
...
*:not(.w-hwrapper) > .w-btn-wrapper:not([class*="mobiles_align_none"]):not(.align_none) {
	display: block;
	margin-inline-end: 0;
	}
...
.g-cols.via_flex.type_default > .wpb_column.stretched:first-of-type {
	margin-inline-start: min( calc( var(--site-content-width) / 2 + <?= $header_width_px ?>px / 2 + 1.5rem - 50vw), -1rem );
	}
.g-cols.via_flex.type_default > .wpb_column.stretched:last-of-type {
	margin-inline-end: min( calc( var(--site-content-width) / 2 + <?= $header_width_px ?>px / 2 + 1.5rem - 50vw), -1rem );
	}
...
.g-cols.via_flex.type_boxes > .wpb_column.stretched:first-of-type {
	margin-inline-start: min( calc( var(--site-content-width) / 2 + <?= $header_width_px ?>px / 2 - 50vw ), -2.5rem );
	}
.l-main .alignfull, /* Full width for Gutenberg blocks */
.w-separator.width_screen,
.g-cols.via_grid > .wpb_column.stretched:last-of-type,
.g-cols.via_flex.type_boxes > .wpb_column.stretched:last-of-type {
	margin-inline-end: min( calc( var(--site-content-width) / 2 + <?= $header_width_px ?>px / 2 - 50vw ), -2.5rem );
	}
}
...
if ( $with_shop AND us_get_option( 'product_gallery' ) == 'slider' ) {
	...
	if ( us_get_option( 'product_gallery_thumbs_pos' ) == 'bottom' ) {
		...
		echo '.woocommerce-product-gallery--columns-' . $cols . ' li { width:' . sprintf( '%0.4F', 100 / $cols ) . '%; }';
	} else {
